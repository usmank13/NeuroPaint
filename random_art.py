...
class NeuralNet:
    def __init__(self, layers_dims, activations_fnc,  type_="classif"):
        ...
        for i in range(len(layers_dims)-1):
            ...
            # self.W[i] = np.ones(layers_dims[i], layers_dims[i+1])
...
# def args_parser(height, width, colormode, alpha, n_images, n_depth, n_size, activation, z1, z2):
#     parser = argparse.ArgumentParser(description="Generate random art with a deep neural network")
    
#     parser.add_argument("-img_height", metavar=height, type=int, default=256,
#                         help="Image height of created random art. Default is 512")
    
#     parser.add_argument("-img_width", metavar=width, type=int, default=256,
#                         help="Image width of created random art. Default is 512")
    
#     parser.add_argument("-colormode", metavar=colormode, type=str, default="RGB",
#                         help="How image color should be generated. Options are ['BW', 'RGB', 'CMYK', 'HSV', 'HSL']. By default this value is 'RGB'")
        
#     parser.add_argument("-alpha", metavar=alpha, type=str, default="False",
#                         help="Whether or not to add a alpha channel for the image. Default is False")
    
#     parser.add_argument("-n_images", metavar=n_images, type=int, default=1,
#                         help="Number of images to generate. Default is 1")
        
#     parser.add_argument("-n_depth", metavar=n_depth, type=int, default=3,
#                         help="Number of layers for the neural network. Default is 10")
    
#     parser.add_argument("-n_size", metavar=n_size, type=int, default=10,
#                         help="Number of neurons in each hidden layer. Default is 15")
    
#     parser.add_argument("-activation", metavar=activation, type=str, default="tanh",
#                         help="Activation function to apply on the hidden layers. Default is 'tanh'")
        
#     parser.add_argument("-z1", metavar=z1, type=float, default=-0.618,
#                         help="Input variable 1 to insert determinism into the random art. The value should be between -1 and 1. Default is -0.618")
        
#     parser.add_argument("-z2", metavar=z2, type=float, default=+0.618,
...
def args_parser():
    ...
    parser.add_argument("-img_height", metavar="", type=int, default=256,
                        help="Image height of created random art. Default is 256")
    
    parser.add_argument("-img_width", metavar="", type=int, default=256,
                        help="Image width of created random art. Default is 256")
    ...
    parser.add_argument("-n_depth", metavar="", type=int, default=6,
                        help="Number of layers for the neural network. Default is 10")
    
    parser.add_argument("-n_size", metavar="", type=int, default=10,
                        help="Number of neurons in each hidden layer. Default is 15")
    
    parser.add_argument("-activation", metavar="tanh", type=str, default="tanh",
                        help="Activation function to apply on the hidden layers. Default is 'tanh'")
...
def info_print(args):
    ...
    """
    print(37*"-")
    print("BCI Art with Deep Neural Networks:")
    print(37*"-")
...
def main():
    ...
    #pull the alpha/beta ratio. If it's above 1.75, set the color to RGB
    # If it's below 1.75, set the color to HSV 


    # Parameters we are changing 
    # colormode= args.colormode
    n_size = 10
    n_depth = num_blinks

    # set these based on coefficients from the brain waves we record
    z1 = -1*ab_ratio / 2
    z2 = ab_ratio/2
    ...
    for i in range(n_images):
        ...
        save_path = "final_results/r4_generated{}.png".format(i+3)
...
# we can keep a while loop running here. Keep taking user input to keep the program running 
#perhaps with a time delay
if __name__ == "__main__":
    # perhaps a while loop with parameters passed in 
    main()